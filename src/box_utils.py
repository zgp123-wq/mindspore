...
if hasattr(config, 'use_anchor_generator') and config.use_anchor_generator:
    default_boxes = make_anchor_generator_paa()
    default_boxes = np.array(default_boxes, dtype='float32')
    default_boxes_ltrb = np.array(tuple(to_ltrb(*i) for i in default_boxes), dtype='float32')
else:
...
def paa_bboxes_encode(boxes):
    ...
    pre_scores = np.zeros((config.num_paa_boxes), dtype=np.float32)
    t_boxes = np.zeros((config.num_paa_boxes, 4), dtype=np.float32)
    t_label = np.zeros((config.num_paa_boxes), dtype=np.int64)
    for bbox in boxes:
        label = int(bbox[4])
    ...
    # Transform to ltrb.
    bboxes = np.zeros((config.num_paa_boxes, 4), dtype=np.float32)
