...
from .fpn import resnet50_fpn,resnet101_fpn
...
class FlattenConcat(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.num_paa_boxes = config.num_paa_boxes
    ...
    def construct(self, inputs):
        ...
        return F.reshape(res, (batch_size, self.num_paa_boxes, -1))

def ClassificationModel(in_channel, num_anchors, kernel_size=3,
                        stride=1, pad_mode='same', num_classes=81, feature_size=256):
    conv1 = nn.Conv2d(in_channel, feature_size, kernel_size, stride, pad_mode)
    conv2 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
    conv3 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
    conv4 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
    conv5 = nn.Conv2d(feature_size, num_anchors * 4, kernel_size, stride,pad_mode)
    return nn.SequentialCell([conv1, nn.ReLU(), conv2, nn.ReLU(), conv3, nn.ReLU(), conv4, nn.ReLU(), conv5])

def RegressionModel(in_channel, num_anchors, kernel_size=3, stride=1, pad_mode='same', feature_size=256):
    conv1 = nn.Conv2d(in_channel, feature_size, kernel_size, stride, pad_mode)
    conv2 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
    conv3 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
    conv4 = nn.Conv2d(feature_size, feature_size, kernel_size, stride, pad_mode)
...
class paaWithLossCell(nn.Cell):
    def __init__(self, network, config):
        super(paaWithLossCell, self).__init__()
...
class PaaResNet50Fpn(nn.Cell):
    def __init__(self, config):
        super(PaaResNet50Fpn, self).__init__()
        self.multi_box = MultiBox(config)
...
class PaaResNet101Fpn(nn.Cell):
    def __init__(self, config):
        super(PaaResNet101Fpn, self).__init__()
        self.multi_box = MultiBox(config)
        self.activation = ops.Sigmoid()
        self.feature_extractor = resnet101_fpn()
...
class paaInferWithDecoder(nn.Cell):
    """
    ...
    def __init__(self, network, default_boxes, config):
        super(paaInferWithDecoder, self).__init__()
        self.network = network
