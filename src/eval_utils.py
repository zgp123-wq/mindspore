...
from src.dataset import create_paa_dataset
import os
...
def apply_eval(eval_param_dict):
    ...
    prefix = "paa_eval.mindrecord"
    mindrecord_dir = config.mindrecord_dir
    mindrecord_file = os.path.join(mindrecord_dir, prefix + "0")
    ds = create_paa_dataset(mindrecord_file, batch_size=1, repeat_num=1, is_training=False)
    anno_json = eval_param_dict["anno_json"]
    coco_metrics = COCOMetrics(anno_json=anno_json,
                               classes=config.coco_classes,
                               num_classes=config.num_classes,
                               max_boxes=config.max_boxes,
                               nms_thershold=config.nms_thershold,
...
class COCOMetrics:
    ...
    def __init__(self, anno_json, classes, num_classes, min_score, nms_thershold, max_boxes):
        self.num_classes = num_classes
        self.classes = classes
        self.min_score = min_score
        self.nms_thershold = nms_thershold
    ...
    def update(self, batch):
        ...
        for c in range(1, self.num_classes):
            ...
            if score_mask.any():
                nms_index = apply_nms(class_boxes, class_box_scores, self.nms_thershold, self.max_boxes)
