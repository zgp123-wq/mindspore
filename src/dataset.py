...
from .box_utils import jaccard_numpy, paa_bboxes_encode
...
def preprocess_fn(img_id, image, box, is_training):
    ...
    def _data_aug(image, box, is_training, image_size=(600, 600)):
        ...
        box, label, num_match = paa_bboxes_encode(box)
...
def voc_data_to_mindrecord(mindrecord_dir, is_training, prefix="paa.mindrecord", file_num=8):
    ...
    writer.add_schema(paa_json, "paa_json")
...
def data_to_mindrecord_byte_image(dataset="coco", is_training=True, prefix="paa.mindrecord", file_num=8):
...
def create_paa_dataset(mindrecord_file, batch_size, repeat_num, device_num=1, rank=0,
                             is_training=True, num_parallel_workers=8):
    """Create paa dataset with MindDataset."""
    ...
    ds = ds.map(operations=trans, input_columns=["image"], python_multiprocessing=True,
                num_parallel_workers=num_parallel_workers)
    ds = ds.batch(batch_size, drop_remainder=True)
    return ds


def create_mindrecord(dataset="coco", prefix="paa.mindrecord", is_training=True):
    print("Start create dataset!")

    # It will generate mindrecord file in config.mindrecord_dir,
    # and the file name is paa.mindrecord0, 1, ... file_num.
    mindrecord_dir = config.mindrecord_dir
